...
1. **Fetch Glossary**: Call `getGlossaryTerms` with `targetLanguageCode` = the derived language (e.g., da-DK).
...
1. **Build App**: Run `al_build` to ensure the generated `g.xlf` is up to date.
2. **Refresh**: Run `refreshXlf` to sync the target file with `g.xlf`.
...
<BatchWorkflow>
Precondition per file: `refreshXlf` has been executed successfully.
...
4. Collect batch and call `saveTranslatedTexts` with `targetState="translated"`.
5. Immediately re-call `getTextsToTranslate` to check remaining untranslated.
</BatchWorkflow>

<CompletionCriteria>
A file is complete when `getTextsToTranslate` returns zero items after a save cycle. Then run `refreshXlf` one final time. If new units appear, resume BatchWorkflow.
</CompletionCriteria>
